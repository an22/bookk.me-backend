...
internal class DeclineAppointmentTest {
    ...
    fun `should return unprocessable entity when already cancelled`() = routeTest {
        ...
        coEvery { useCase.invoke(userId, cancellation) } returns Result.failure(CancelAppointment.Error.AlreadyCancelled())
    ...
    fun `should return unprocessable entity when already completed`() = routeTest {
        ...
        coEvery { useCase.invoke(userId, cancellation) } returns Result.failure(CancelAppointment.Error.AlreadyCompleted())
    ...
    fun `should return unauthorized when cancelling appointment without authentication`() = routeTest {
        ...
        whenn()
        val client = createTestClient()
        val response = client.post(Api.Appointment.Cancel(parent = Api.Appointment(parent = Api()), id = Uuid.random())) {
