...
**Keep the database ER diagrams current.** `docs/database-er-diagrams.md` documents every microservice's schema as Mermaid ER diagrams, plus a cross-service overview of logical (non-FK) references. Any change to an ORM table — new/removed table, column, or relationship — must update that service's diagram (and the cross-service overview, if a logical reference changed) in the same change.
