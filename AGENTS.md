...
- **Adding a field to an existing `EventStreaming.Event` must pin ProtoBuf numbers.** Events have no `@ProtoNumber`s, so numbers are declaration order — and the body property `override val topic: String = TOPIC` is serialized too, numbered after every constructor parameter. A new constructor parameter therefore shifts `topic` and breaks decoding of in-flight messages. Pin `topic` to its current number and give the new field the next one with a default (`BusinessEvent.EmployeePermissionsChanged`: `topic` `@ProtoNumber(5)`, `suspended` `@ProtoNumber(6) = false`), and prove old↔new decoding with a legacy-shaped `@Serializable` copy (`BusinessEventTest`).
